...
TMP=$(mktemp -d)
SOURCES=(armadillo_bar.swift ArmadilloClippyWindow.swift ClippyBubblePanel.swift ArmadilloAskWindow.swift ArmadilloTTS.swift ArmadilloCallDetector.swift)
for t in "${TARGETS[@]}"; do
  swiftc "${SOURCES[@]}" -O \
    -target "$t" \
